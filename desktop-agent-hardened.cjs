...
/**
 * SmartBlueprint Pro - Hardened Desktop Agent (CommonJS)
...
function validateEnvironment() {
  ...
  // Check required dependencies with graceful fallback
...
class ConfigurationManager {
  ...
  applyEnvironmentOverrides(config) {
    // Smart URL detection from environment
    if (process.env.WS_URL) {
      config.serverUrl = process.env.WS_URL;
      console.log(`[Agent] Using WS_URL: ${config.serverUrl}`);
    }
    ...
    if (process.env.SMARTBLUEPRINT_HOST) {
      ...
      config.serverUrl = `${protocol}://${host}/ws`;
      console.log(`[Agent] Using SMARTBLUEPRINT_HOST: ${config.serverUrl}`);
    }
    ...
    if (process.env.REPLIT_DOMAINS) {
      ...
      console.log(`[Agent] Detected Replit environment: ${config.serverUrl}`);
    }
    ...
    for (const [envVar, configPath] of Object.entries(envMappings)) {
      if (process.env[envVar]) {
        if (typeof configPath === 'function') {
          const key = envVar.toLowerCase().replace('_', '');
          config[key] = configPath(process.env[envVar]);
...
Usage: node desktop-agent-hardened.cjs [options]
...
Environment Variables:
  ...
  REPLIT_DOMAINS              Auto-detects Replit environment
...
Examples:
  node desktop-agent-hardened.cjs --server wss://api.smartblueprint.io/ws
  WS_URL=ws://192.168.1.100:5000/ws node desktop-agent-hardened.cjs
  node desktop-agent-hardened.cjs --agent-id my-desktop-001
`);
  }
}
...
class HardenedDesktopAgent {
  ...
  async start() {
    try {
      ...
      console.error('[Agent] Retrying connection in 10 seconds...');
