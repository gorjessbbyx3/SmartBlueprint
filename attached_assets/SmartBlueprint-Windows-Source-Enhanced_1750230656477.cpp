// SmartBlueprint Pro - Windows Source Code Package
...
#ifdef _WIN32
#include <winsock2.h>
#include <ws2tcpip.h>  // For advanced socket features
#include <winsock2.h>
#include <ws2tcpip.h>
#include <iphlpapi.h>
#include <conio.h>
...
public:
    ...
    void showDashboard() {
        ...
        for (const auto& device : devices) {
            if (!device->isOnline || device->rssi < -65) {
                std::cout << "⚠️  Device " << device->hostname 
                          << ": Signal Weak or Offline — Confidence: "
                          << (device->isOnline ? "70%" : "92%") << "\n";
            }
        }
