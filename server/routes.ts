...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Desktop Agent Download - ensure this is before any catch-all routes
  app.get('/api/download/desktop-agent', (req, res) => {
    try {
      ...
      // Always use the fallback content for now to ensure it works
      ...
      // Fallback: generate installer content directly
...
`;
      ...
      res.status(500).json({ message: "Failed to download desktop agent" });
    }
  });
